/*
  # Fix User XP RPC Function

  1. Updates
    - Create a new RPC function to properly handle user XP updates
    - Ensure all values are properly validated as integers
    - Add proper error handling and logging
    - Fix the daily login check to properly handle date comparisons
...
BEGIN
  ...
  IF p_value IS NULL OR p_value <= 0 THEN
    RAISE EXCEPTION 'Value must be a positive integer, got: %', p_value;
  END IF;
